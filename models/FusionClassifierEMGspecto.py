...
class FusionClassifierEMGspecto(nn.Module):
    def __init__(self, num_classes=20):
        super(FusionClassifierEMGspecto, self).__init__()
       
        self.EMGClassifier = EMGClassifier()
        self.LeNet5 = LeNet5()

        self.dropout = nn.Dropout(p=0.3)
        self.fc = nn.Linear(84+50, num_classes)

    def forward(self, emg, specto):

        emg = self.EMGClassifier(emg)
        specto = self.LeNet5(specto)

        conc = torch.cat((emg[0], specto), dim=1)
        x = self.fc(conc)
