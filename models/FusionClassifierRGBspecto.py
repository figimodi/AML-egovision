...
class FusionClassifierRGBspecto(nn.Module):
    def __init__(self, num_classes=20):
        super(FusionClassifierRGBspecto, self).__init__()
       
        self.RGBClassifier = RGBClassifier()
        self.LeNet5 = LeNet5()

        self.dropout = nn.Dropout(p=0.3)
        self.fc = nn.Linear(84+512, num_classes)

    def forward(self, rgb, specto):

        rgb = self.RGBClassifier(rgb)
        specto = self.LeNet5(specto)

        conc = torch.cat((rgb[0], specto), dim=1)
        x = self.fc(conc)
