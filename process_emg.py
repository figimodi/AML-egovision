...
class ProcessEmgDataset():
    ...
    def __init__(self, input_folder:str, output_folder:str):
        ...
        self.specto_folder = None
        self.current_step = 1
        self.current_emg_folder = input_folder
        self.current_split_folder = input_folder
        self.input_folder = input_foledr
        self.output_folder = output_folder

    def __trim_samples__(left_timestamps:list, left_readings:list, right_timestamps:list, right_readings:list, time_interval:int=5):
    ...
    def __update_split_files__(path_data):
        train = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['train'])))
        test = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['test'])))
        
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            data = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename)))
        ...
        self.current_split_folder = f"tmp{self.current_step}"
        self.current_step += 1
        train.to_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['train']))
        test.to_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['test']))

    def __low_pass_filter__(data, fs:float=10, cut_frequency:float=5., filter_order:int=2) -> pd.DataFrame:
        print("Low pass filtering dataset...")
        ...
        for side in ['myo_left_readings', 'myo_right_readings']:
                ...
                for j in range(8):
                    np_side_data[:,j] = signal.filtfilt(np_side_data[:,j], b, a, data)
    ...
    def __normalize__(data) -> pd.DataFrame: 
    ...
    def __scale__(data) -> pd.DataFrame:
    ...
    def __save_spectogram__(specgram_l, specgram_r, name, resize_factor=.25):
    ...
    def calculate_stats():
        split_train = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['train'])))
        samples = {}
        dataset_train = []

        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            agent = filename[:5]
            samples_agent = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename)))
    ...
    def delete_temps() -> None:
        folders = []
        for item in os.listdir(FOLDERS['data']):
            if os.path.isdir(os.path.join(FOLDERS['data'], item)):
                folders.append(item)

        for folder in folders:   
            if folder == self.input_folder or folder == self.output_folder:
                continue
            for filename in os.listdir(os.path.join(FOLDERS['data'], folder)):
                os.remove(os.path.join(FOLDERS['data'], folder, filename))
            os.rmdir(os.path.join(FOLDERS['data'], folder))

        for item in os.listdir(FOLDERS['split']):
            if os.path.isdir(os.path.join(FOLDERS['split'], item)):
                folders.append(item)
    ...
    def augment_dataset() -> None:
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            ...
            for i, row in data.iterrows():
                left_timestamps_chunks, right_timestamps_chunks, left_readings_chunks, right_readings_chunks = self.__trim_samples__(row['myo_left_timestamps'], row['myo_left_readings'], row['myo_right_timestamps'], row['myo_right_readings'])
            ...
            self.current_emg_folder = f"tmp{self.current_step}"
            self.current_step += 1
            new_data.to_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename))
    ...
    def resample() -> None:
        try:
            ...
            for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
                dataframe = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename)))
                ...
                self.current_emg_folder = f'tmp{self.current_step}'
                self.current_step +=1
                dataframe.to_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename))
    ...
    def padding() -> None:
        ...
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            ...
            samples_agent = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename)))
            samples[agent] = samples_agent
            max_sizes[agent] = max([max(len(sample['myo_left_readings']), len(sample['myo_right_readings'])) for sample in samples[agent]])
        
        max_lenght = max([max_size for max_size in max_sizes.values()])

        for agent, dataframe in samlpes.items():
            dataframe = dataframe.apply(lambda row: self.__pad_item__(row, max_length), axis=1)
            dataframe.to_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder), f"{agent}.pkl")

        self.current_emg_folder = f"tmp{self.current_step}"
        self.current_step += 1

    def pre_processing(operations:list=['filter', 'scale', 'normalize'], fs:float=10., cut_frequency:float=5., filter_order:int=2) -> None:
        map_functions = {
        ...
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            ...
            self.current_emg_folder = f"tmp{self.current_step}"
            self.current_step += 1
            data.to_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename))

        print("Dataset was correctly preprocessed")            

    def generate_spectograms(save_spectrograms:bool=True) -> None:
        ...
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_data)):
            cur_values = []
            agent = filename[:5]
            emg_annotations = pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_data, filename))
            ...
            cur_df.to_pickle(os.path.join(FOLDERS['data'], self.specto_folder, filename))

        print(f'dataset produced the spectogram counterpart succesfully')

    def generate_rgb() -> None:
        for filename in os.listdir(os.path.join(FOLDERS['data'], self.current_emg_folder)):
            try: 
                data = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, filename)))
                new_data = []
                ...
                # Save the DataFrame to a pickle file
                self.rgb_folder = f"tmp{self.current_step}"
                self.current_step += 1
                new_data.to_pickle(os.path.join(self.rgb_folder, filename))
            except Exception as e:
                print(f"Error: {e}")

        print(f'dataset produced the RGB counterpart succesfully')

    def merge_pickles() -> None:
        agents = {}

        for filename in os.listdir(os.path.join(FOLDERS['data'], self.rgb_folder)):
            agent = filename[:5]
            agents[agent]['rgb_file'] = filename

        for filename in os.listdir(os.path.join(FOLDERS['data'], self.specto_folder)r):
            agent = filename[:5]
        ...
        for agent, files in agents.items():
            emg = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.current_emg_folder, files['emg_file'])))
            rgb = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.self.rgb_folder, files['rgb_file'])))
            specto = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['data'], self.specto_folder, files['spectograms_file'])))
            ...
            emg['start'] = emg['start'].map(lambda x: __remove_t0_time__(x, t0=t0))
            emg['stop'] = emg['stop'].map(lambda x: __remove_t0_time__(x, t0=t0))
            emg['myo_left_timestamps'] = emg['myo_left_timestamps'].map(lambda x: __remove_t0_time__(x, t0=t0))
            emg['myo_right_timestamps'] = emg['myo_right_timestamps'].map(lambda x: __remove_t0_time__(x, t0=t0))
            rgb['start_frame'] = rgb['start_frame'].map(lambda x: __remove_t0_frame__(x, t0=t0))
            rgb['stop_frame'] = rgb['stop_frame'].map(lambda x: __remove_t0_frame__(x, t0=t0))
            ...
            self.current_emg_folder = self.output_folder
            final.to_pickle(os.path.join(FOLDERS['data'], self.output_folder, f'{agent}.pkl'))

            print("Pickles were correctly merged")

    def balance_splits(train_split_proportion:float=.05) -> None:
        split_train = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['train'])))
        split_test = pd.DataFrame(pd.read_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['test'])))
        samples = {}
        dataset_train = []
        dataset_test = []
        ...
        self.current_split_folder = f"tmp{self.current_step}"
        self.current_step += 1
        split_train.to_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['train']))
        split_test.to_pickle(os.path.join(FOLDERS['split'], self.current_split_folder, self.split_files['test']))

        print('Split files were correctly balanced')

if __name__ == '__main__':
    processing = ProcessEmgDataset(input_folder='base', output_folder='final')
    ...
    processing.generate_spectograms()
