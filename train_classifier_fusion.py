...
from utils.args_fusion import args
...
def main():
    ...
    # the models are wrapped into the ActionRecognitionFusion task which manages all the training steps
    action_classifier = tasks.ActionRecognitionFusion("action-classifier", models, args.batch_size,
                                                args.total_batch, args.models_dir, num_classes,
                                                1, args_mod.models, args=args )
...
def validate(model, val_loader, device, it, num_classes):
    ...
    with open(os.path.join(args.log_dir, f'val_precision_{modalities[0]}.txt'), 'a+') as f:
        f.write("[%d/%d]\tAcc@top1: %.2f%%\n" % (it, args_mod.train.num_iter, test_results['top1']))
